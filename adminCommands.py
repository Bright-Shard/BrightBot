...
class admin(commands.Cog, name="Admin Commands"):
  ...
  async def has_admin_role(ctx):
    ...
    if 'admin_commands' in targetRoleList:
      return True
    else:
      await ctx.send("You need the `admin_commands` role to run this command.")
      return False
  ...
  @commands.check(has_admin_role) # Make sure whoever is running the command has the admin_commands role
